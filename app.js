...
COLUMN_ALIASES.proposal = ["제안", "제안수", "제안횟수", "권유", "상담", "proposal"];
COLUMN_ALIASES.payment = ["결제", "결제수", "결제횟수", "구매", "payment"];
COLUMN_ALIASES.note = ["비고", "메모", "내용", "특이사항", "할인", "remark", "note"];
...
function cacheElements() {
  [
    ...
    "categoryCaption",
    "packageCaption",
    "comparisonDoctorCaption",
    "comparisonPackageCaption",
    "comparisonDoctorTable",
    "comparisonPackageTable",
    "selectedPackageTitle",
    "clearPackageSelection",
    "packageDetailHead",
    "packageDetailBody",
    "dailyDoctorCaption",
    "doctorCategoryCaption",
    "dailyDoctorChart",
    "doctorCategoryChart",
    "recordCaption",
    "configCaption",
...
function bindEvents() {
  ...
  els.clearPackageSelection.addEventListener("click", () => {
    state.selectedPackageKey = "";
    render();
  });
  document.addEventListener("click", (event) => {
    const packageRow = event.target.closest("[data-package-key]");
    if (!packageRow) return;
    state.selectedPackageKey = packageRow.dataset.packageKey || "";
...
function saveRuntimeConfig() {
  localStorage.setItem(
    ...
    JSON.stringify({
      ...
      columnMap: {
        date: els.dateColumnInput.value,
        doctor: els.doctorColumnInput.value,
        category: els.categoryColumnInput.value,
        packageName: els.packageColumnInput.value,
        amount: els.amountColumnInput.value,
      },
    }),
  );
  populateConfigForm();
...
async function loadData() {
  ...
  state.columns = applyColumnOverrides(detectColumns(state.rawRows), config.columnMap);
  populateColumnSelectors();
  resetDateFilters();
...
function detectColumns(rows) {
  ...
  const found = Object.entries(COLUMN_ALIASES).reduce((result, [key, aliases]) => {
    ...
    result[key] = (exact || partial)?.original || "";
    return result;
  }, {});

  const sampleRows = rows.slice(0, 250);
  if (!found.date) found.date = bestColumnByScore(headers, sampleRows, (value) => (parseDate(value) ? 3 : 0));
  if (!found.amount) {
    found.amount = bestColumnByScore(headers, sampleRows, (value) => {
      const number = parseNumber(value);
      return Math.abs(number) > 0 ? 2 : 0;
    });
  }
...
function bestColumnByScore(headers, rows, scoreValue) {
  let best = "";
  let bestScore = 0;

  headers.forEach((header) => {
    const score = rows.reduce((total, row) => total + scoreValue(row[header]), 0);
    if (score > bestScore) {
      best = header;
      bestScore = score;
    }
...
function populateFilterOptions() {
  state.filters.doctor = fillSelect(els.doctorFilter, uniqueValues("doctor"), "전체 원장", state.filters.doctor);
  state.filters.category = fillSelect(els.categoryFilter, uniqueValues("category"), "전체 분류", state.filters.category);
  state.filters.packageName = fillSelect(els.packageFilter, uniqueValues("packageName"), "전체 패키지", state.filters.packageName);
  els.granularityFilter.value = state.filters.granularity;
}

function fillSelect(select, options, allLabel, currentValue) {
  const currentKey = currentValue === "__all" ? "__all" : canonicalText(currentValue);
  select.innerHTML = "";
  select.append(new Option(allLabel, "__all"));
  options.forEach((value) => select.append(new Option(value, canonicalText(value))));
  const optionValues = [...select.options].map((option) => option.value);
  select.value = optionValues.includes(currentKey) ? currentKey : "__all";
  return select.value;
}

function applyColumnOverrides(detected, columnMap = {}) {
  return {
    ...detected,
    ...Object.fromEntries(
      Object.entries(columnMap || {}).filter(([, value]) => value && value !== "__auto"),
    ),
  };
}
...
function populateColumnSelectors() {
  ...
  [
    ["dateColumnInput", "date"],
    ["doctorColumnInput", "doctor"],
    ["categoryColumnInput", "category"],
    ["packageColumnInput", "packageName"],
    ["amountColumnInput", "amount"],
  ].forEach(([elementId, columnKey]) => {
    const select = els[elementId];
    if (!select) return;
    const current = state.columns[columnKey] || "__auto";
    select.innerHTML = "";
    select.append(new Option("자동 인식", "__auto"));
    headers.forEach((header) => select.append(new Option(header, header)));
    select.value = headers.includes(current) ? current : "__auto";
  });

  if (els.configCaption && headers.length) {
    const dateLabel = state.columns.date || "미인식";
    const amountLabel = state.columns.amount || "미인식";
    els.configCaption.textContent = `날짜: ${dateLabel} / 금액: ${amountLabel}`;
  }
}
...
function uniqueValues(columnKey) {
  ...
  return [...values.values()].sort((a, b) => a.localeCompare(b, "ko-KR"));
}

function render() {
  const rows = getFilteredRows();
  const previousRows = getPreviousFilteredRows();
  ...
  const doctorComparisons = compareGroups(rows, previousRows, "doctor");
  const packageComparisons = compareGroups(rows, previousRows, "packageName");
  ...
  renderComparisonTable(els.comparisonDoctorTable, doctorComparisons, "doctor");
  renderComparisonTable(els.comparisonPackageTable, packageComparisons, "packageName");
  renderPackageDetailTable(rows);
  renderMultiLineChart(els.dailyDoctorChart, rows, "doctor");
  renderStackedBarChart(els.doctorCategoryChart, rows, "doctor", "category");
  ...
  els.comparisonDoctorCaption.textContent = comparisonCaption(previousRows);
  els.comparisonPackageCaption.textContent = comparisonCaption(previousRows);
  els.dailyDoctorCaption.textContent = `${rows.length.toLocaleString("ko-KR")}건`;
  els.doctorCategoryCaption.textContent = `${doctors.length.toLocaleString("ko-KR")}명`;
  els.recordCaption.textContent = `${rows.length.toLocaleString("ko-KR")}행`;
}

function getFilteredRows() {
  const { start, end } = getActiveDateRange();
  return filterRowsByRange(start, end);
}

function getPreviousFilteredRows() {
  const { start, end } = getActiveDateRange();
  if (!start || !end) return [];
  const previous = getPreviousRange(start, end);
  return filterRowsByRange(previous.start, previous.end);
}
...
function isWholeMonth(start, end) {
  ...
  return start.getDate() === 1 && end.getDate() === lastDay && start.getMonth() === end.getMonth();
}

function comparisonCaption(previousRows) {
  if (!previousRows.length) return "비교 데이터 없음";
  return `비교 ${previousRows.length.toLocaleString("ko-KR")}건`;
}
...
function matchesFilter(row, key, value) {
  ...
  const rowKey = canonicalText(row[column]);
  const filterKey = canonicalText(value);
  return rowKey === filterKey || (rowKey.length > 1 && filterKey.length > 1 && (rowKey.includes(filterKey) || filterKey.includes(rowKey)));
}
...
function compareGroups(currentRows, previousRows, key) {
  const current = groupRows(currentRows, key);
  const previous = groupRows(previousRows, key);
  const previousMap = new Map(previous.map((item) => [canonicalText(item.name), item]));

  return current.map((item) => {
    const previousItem = previousMap.get(canonicalText(item.name));
    const previousRevenue = previousItem?.revenue || 0;
    const change = item.revenue - previousRevenue;
    const changeRate = previousRevenue ? (change / previousRevenue) * 100 : null;
    return {
      ...item,
      previousRevenue,
      change,
...
function renderComparisonTable(target, rows, key) {
  if (!rows.length) {
    target.innerHTML = `<tr><td colspan="5">${emptyState()}</td></tr>`;
    return;
  }
  ...
  target.innerHTML = rows
    .slice(0, 30)
    .map((item) => {
      const deltaClass = item.change > 0 ? "is-up" : item.change < 0 ? "is-down" : "";
      const packageAttr = key === "packageName" ? ` data-package-key="${escapeHtml(canonicalText(item.name))}"` : "";
      return `
        <tr class="${key === "packageName" ? "clickable-row" : ""}"${packageAttr}>
          <td>${escapeHtml(item.name)}</td>
          <td class="numeric">${formatWon(item.revenue)}</td>
          <td class="numeric">${item.previousRevenue ? formatWon(item.previousRevenue) : "-"}</td>
          <td class="numeric ${deltaClass}">${formatSignedWon(item.change)}</td>
          <td class="numeric ${deltaClass}">${formatDeltaRate(item.changeRate)}</td>
        </tr>
      `;
    })
    .join("");
...
function renderPackageDetailTable(rows) {
  const packageColumn = state.columns.packageName;
  const filteredRows = state.selectedPackageKey
    ? rows.filter((row) => canonicalText(row[packageColumn]) === state.selectedPackageKey)
    : rows;
  const selectedName = state.selectedPackageKey
    ? cleanText(filteredRows[0]?.[packageColumn]) || "선택 패키지"
    : "전체 패키지";

  els.selectedPackageTitle.textContent = `${selectedName} 결제 내역`;
  els.clearPackageSelection.style.visibility = state.selectedPackageKey ? "visible" : "hidden";

  const columns = [
    ["날짜", state.columns.date, (row, column) => formatDateCell(row[column])],
    ["이름", state.columns.patient, (row, column) => cleanText(row[column])],
    ["패키지", state.columns.packageName, (row, column) => cleanText(row[column])],
    ["제안", state.columns.proposal, (row, column) => cleanText(row[column])],
    ["결제", state.columns.payment, (row, column) => cleanText(row[column])],
    ["원장명", state.columns.doctor, (row, column) => cleanText(row[column])],
    ["비고", state.columns.note, (row, column) => cleanText(row[column])],
    ["금액", state.columns.amount, (row, column) => formatWon(parseNumber(row[column]))],
  ].filter(([, column]) => column);

  if (!filteredRows.length || !columns.length) {
    els.packageDetailHead.innerHTML = "";
    els.packageDetailBody.innerHTML = `<tr><td>${emptyState()}</td></tr>`;
    return;
  }

  const sortedRows = [...filteredRows].sort((a, b) => (parseDate(b[state.columns.date]) || 0) - (parseDate(a[state.columns.date]) || 0));
  els.packageDetailHead.innerHTML = `<tr>${columns
    .map(([label]) => `<th${label === "금액" ? ' class="numeric"' : ""}>${label}</th>`)
    .join("")}</tr>`;
  els.packageDetailBody.innerHTML = sortedRows
    .slice(0, 120)
    .map(
      (row) => `
        <tr>${columns
          .map(([label, column, formatter]) => `<td${label === "금액" ? ' class="numeric"' : ""}>${escapeHtml(formatter(row, column))}</td>`)
          .join("")}</tr>
      `,
    )
    .join("");
...
function renderMultiLineChart(target, rows, key) {
  const dateColumn = state.columns.date;
  const keyColumn = state.columns[key];
  if (!rows.length || !dateColumn || !keyColumn) {
    target.innerHTML = emptyState();
    return;
  }

  const totalsByKey = groupRows(rows, key).slice(0, 6);
  const keys = totalsByKey.map((item) => item.name);
  const days = [...new Set(rows.map((row) => parseDate(row[dateColumn])).filter(Boolean).map(toDateInput))].sort();
  ...
  const series = keys.map((name) => ({
    name,
    values: Array.from({ length: days.length }, () => 0),
  }));
  const seriesMap = new Map(series.map((item) => [canonicalText(item.name), item]));

  rows.forEach((row) => {
    const date = parseDate(row[dateColumn]);
    const name = cleanText(row[keyColumn]);
    if (!date || !seriesMap.has(canonicalText(name))) return;
    seriesMap.get(canonicalText(name)).values[dayIndex.get(toDateInput(date))] += getAmount(row);
  });

  const width = 900;
  const height = 430;
  const margin = { top: 36, right: 28, bottom: 54, left: 74 };
  const innerWidth = width - margin.left - margin.right;
  const innerHeight = height - margin.top - margin.bottom;
  const max = Math.max(...series.flatMap((item) => item.values), 1);
  const colors = ["#0f766e", "#b7791f", "#a33a4a", "#4f46e5", "#7c3aed", "#475569"];
  ...
  const paths = series
    .map((item, seriesIndex) => {
      const path = item.values
        .map((value, index) => {
          const x = margin.left + (days.length === 1 ? innerWidth / 2 : (index / (days.length - 1)) * innerWidth);
          const y = margin.top + innerHeight - (value / max) * innerHeight;
          return `${index ? "L" : "M"} ${x} ${y}`;
        })
        .join(" ");
      return `<path d="${path}" fill="none" stroke="${colors[seriesIndex % colors.length]}" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><title>${escapeHtml(item.name)}</title></path>`;
    })
    .join("");

  target.innerHTML = `
    <svg viewBox="0 0 ${width} ${height}" role="img" aria-label="일자별 원장 매출">
      <line x1="${margin.left}" y1="${margin.top + innerHeight}" x2="${width - margin.right}" y2="${margin.top + innerHeight}" stroke="#d9d2c6" />
      <line x1="${margin.left}" y1="${margin.top}" x2="${margin.left}" y2="${margin.top + innerHeight}" stroke="#d9d2c6" />
      ${paths}
      ${series
        .map(
          (item, index) =>
            `<text x="${margin.left + index * 116}" y="20" class="axis-label" fill="${colors[index % colors.length]}">${escapeHtml(item.name)}</text>`,
        )
        .join("")}
      <text x="${margin.left - 8}" y="${margin.top + 4}" text-anchor="end" class="axis-label">${formatCompactWon(max)}</text>
      <text x="${margin.left - 8}" y="${margin.top + innerHeight}" text-anchor="end" class="axis-label">0</text>
      ${days
        ...
        .map((day, index) => {
          ...
          const x = margin.left + (days.length === 1 ? innerWidth / 2 : (originalIndex / (days.length - 1)) * innerWidth);
          return `<text x="${x}" y="${height - 18}" text-anchor="middle" class="axis-label">${day.slice(5)}</text>`;
        })
        .join("")}
    </svg>
  `;
...
function renderStackedBarChart(target, rows, groupKey, stackKey) {
  const groupColumn = state.columns[groupKey];
  const stackColumn = state.columns[stackKey];
  if (!rows.length || !groupColumn || !stackColumn) {
    target.innerHTML = emptyState();
    return;
  }

  const groups = groupRows(rows, groupKey).slice(0, 8).map((item) => item.name);
  const stacks = groupRows(rows, stackKey).slice(0, 6).map((item) => item.name);
  const bucket = new Map(groups.map((group) => [group, Object.fromEntries(stacks.map((stack) => [stack, 0]))]));

  rows.forEach((row) => {
    const group = cleanText(row[groupColumn]);
    const stack = cleanText(row[stackColumn]);
    if (!bucket.has(group) || !stacks.includes(stack)) return;
    bucket.get(group)[stack] += getAmount(row);
  });

  const width = 900;
  const height = 430;
  const margin = { top: 42, right: 28, bottom: 70, left: 74 };
  const innerWidth = width - margin.left - margin.right;
  const innerHeight = height - margin.top - margin.bottom;
  const totals = groups.map((group) => sum(stacks, (stack) => bucket.get(group)[stack]));
  const max = Math.max(...totals, 1);
  const colors = ["#0f766e", "#b7791f", "#a33a4a", "#4f46e5", "#7c3aed", "#475569"];
  const barWidth = Math.max(28, innerWidth / Math.max(groups.length, 1) - 26);

  target.innerHTML = `
    <svg viewBox="0 0 ${width} ${height}" role="img" aria-label="원장별 분류 매출">
      <line x1="${margin.left}" y1="${margin.top + innerHeight}" x2="${width - margin.right}" y2="${margin.top + innerHeight}" stroke="#d9d2c6" />
      <line x1="${margin.left}" y1="${margin.top}" x2="${margin.left}" y2="${margin.top + innerHeight}" stroke="#d9d2c6" />
      ${stacks
        .map(
          (stack, index) =>
            `<rect x="${margin.left + index * 110}" y="16" width="18" height="12" fill="${colors[index % colors.length]}"></rect><text x="${margin.left + 24 + index * 110}" y="27" class="axis-label">${escapeHtml(stack)}</text>`,
        )
        .join("")}
      ${groups
        .map((group, groupIndex) => {
          const x = margin.left + groupIndex * (innerWidth / groups.length) + 12;
          let y = margin.top + innerHeight;
          const rects = stacks
            .map((stack, stackIndex) => {
              const value = bucket.get(group)[stack];
              const heightValue = (value / max) * innerHeight;
              y -= heightValue;
              return `<rect x="${x}" y="${y}" width="${barWidth}" height="${heightValue}" fill="${colors[stackIndex % colors.length]}"><title>${escapeHtml(group)} / ${escapeHtml(stack)}: ${formatWon(value)}</title></rect>`;
            })
            .join("");
          return `${rects}<text x="${x + barWidth / 2}" y="${height - 24}" text-anchor="middle" class="axis-label">${escapeHtml(group)}</text>`;
        })
        .join("")}
      <text x="${margin.left - 8}" y="${margin.top + 4}" text-anchor="end" class="axis-label">${formatCompactWon(max)}</text>
      <text x="${margin.left - 8}" y="${margin.top + innerHeight}" text-anchor="end" class="axis-label">0</text>
...
function parseDate(value) {
  ...
  const monthMatch = normalized.match(/(\d{4})\s*-\s*(\d{1,2})(?:\s|$)/);
  if (monthMatch) {
    return validDate(Number(monthMatch[1]), Number(monthMatch[2]), 1);
  }
  ...
  if (compact.length >= 8) {
    ...
    if (compactMatch) {
      return validDate(Number(compactMatch[1]), Number(compactMatch[2]), Number(compactMatch[3]));
    }
  }

  const parts = text.match(/\d+/g) || [];
  if (parts.length >= 3 && parts[0].length === 4) {
    return validDate(Number(parts[0]), Number(parts[1]), Number(parts[2]));
  }
  if (parts.length >= 3 && parts[0].length === 2) {
    return validDate(2000 + Number(parts[0]), Number(parts[1]), Number(parts[2]));
  }
  if (parts.length >= 2 && parts[0].length === 2) {
    return validDate(2000 + Number(parts[0]), Number(parts[1]), 1);
  }
